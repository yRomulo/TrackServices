...
import { mainNavigationLinks, secondaryNavigationLinks, serviceNavigationLinks } from "@/data/siteNavigation";

export function SiteHeader() {
  const pathname = usePathname();
  const [navElevated, setNavElevated] = useState(false);
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  const [openDropdown, setOpenDropdown] = useState<"servicos" | null>(null);
  ...
  return (
    <header id="top" className="sticky top-0 z-50">
      ...
      <nav className="flex h-[68px] items-center border-b border-slate-200 bg-white px-[5%]" style={{ boxShadow: navElevated ? "0 2px 18px rgba(0,0,0,.1)" : "0 1px 6px rgba(0,0,0,.06)" }}>
        <div className="mx-auto flex w-full max-w-[1280px] items-center justify-between">
          <Link href="/" className="flex items-center gap-2" onClick={() => { closeMobileMenu(); setOpenDropdown(null); }}>
          ...
          <ul className="hidden items-center gap-1 lg:flex">
            {mainNavigationLinks.map((link) => {
              if (link.href === "/servicos") {
                return (
                  <li key={link.href} className="relative" onMouseEnter={openServicesMenu} onMouseLeave={closeServicesMenu}>
                    <Link href={link.href} className={`flex items-center gap-1 rounded px-3 py-2 text-xs font-semibold uppercase tracking-[0.04em] transition ${isActive(link.href) ? "bg-orange-50 text-[#E8621A]" : "text-slate-700 hover:bg-orange-50 hover:text-[#E8621A]"}`} aria-haspopup="true" aria-expanded={openDropdown === "servicos"}>
                      {link.label}
                      <svg viewBox="0 0 24 24" className="h-3 w-3 stroke-current" fill="none" strokeWidth="2.5" aria-hidden="true"><path d="M6 9l6 6 6-6" /></svg>
                    </Link>
                    <div className={`pointer-events-none absolute left-0 top-full z-50 min-w-[320px] pt-3 ${openDropdown === "servicos" ? "visible" : "invisible"}`}>
                      <div className="pointer-events-auto rounded-b-md border border-slate-200 border-t-[3px] border-t-[#E8621A] bg-white shadow-xl transition" onMouseEnter={openServicesMenu} onMouseLeave={closeServicesMenu}>
                        {serviceNavigationLinks.map((serviceLink) => (
                          <Link key={serviceLink.href} href={serviceLink.href} className="flex items-start gap-2 border-l-2 border-transparent px-4 py-3 text-sm text-slate-700 transition hover:border-l-[#E8621A] hover:bg-orange-50 hover:text-[#E8621A]" onClick={() => setOpenDropdown(null)}>
                            <span>▸</span>
                            <span>
                              <span className="block text-sm font-semibold">{serviceLink.label}</span>
                              <span className="block text-xs text-slate-500">{serviceLink.description}</span>
                            </span>
                          </Link>
                        ))}
                      </div>
              ...
              return (
                <li key={link.href}>
                  <Link href={link.href} className={`rounded px-3 py-2 text-xs font-semibold uppercase tracking-[0.04em] transition ${isActive(link.href) ? "bg-orange-50 text-[#E8621A]" : "text-slate-700 hover:bg-orange-50 hover:text-[#E8621A]"}`}>
                    {link.label}
                  </Link>
                </li>
              );
          ...
          <div className="hidden items-center gap-2 lg:flex">
            {secondaryNavigationLinks.map((link) => (
              <Link key={link.href} href={link.href} className={`rounded px-3 py-2 text-xs font-semibold uppercase tracking-[0.04em] transition ${isActive(link.href) ? "bg-orange-50 text-[#E8621A]" : "text-slate-700 hover:bg-orange-50 hover:text-[#E8621A]"}`}>
                {link.label}
              </Link>
            ))}
            <Link href="/contato" className="rounded bg-[#E8621A] px-4 py-2 text-xs font-bold uppercase tracking-[0.04em] text-white shadow-md transition hover:bg-[#C04E10]">
              Fale Conosco
            </Link>
          </div>
          ...
          <button
            type="button"
            className="inline-flex flex-col gap-1 lg:hidden"
        ...
        {mobileMenuOpen ? (
          <div id="mobile-menu" className="fixed left-0 right-0 top-[68px] z-40 border-b-4 border-b-[#E8621A] bg-white px-5 py-4 shadow-xl lg:hidden">
            <div className="flex flex-col gap-2 text-sm font-semibold text-slate-700">
              {mainNavigationLinks.map((link) => (
                <Link key={link.href} href={link.href} onClick={closeMobileMenu} className="rounded px-2 py-2 hover:bg-orange-50 hover:text-[#E8621A]">
                  {link.label}
                </Link>
              ))}

              <div className="mt-2 rounded border border-slate-200 bg-slate-50 p-3">
                <div className="mb-2 text-[11px] font-bold uppercase tracking-[0.08em] text-[#E8621A]">Divisões</div>
                <div className="flex flex-col gap-1">
                  {serviceNavigationLinks.map((link) => (
                    <Link key={link.href} href={link.href} onClick={closeMobileMenu} className="rounded px-2 py-2 text-sm text-slate-700 hover:bg-white hover:text-[#E8621A]">
                      {link.label}
                    </Link>
                  ))}
                </div>
              ...
              <div className="mt-2 flex flex-col gap-2 rounded border border-slate-200 bg-slate-50 p-3">
                {secondaryNavigationLinks.map((link) => (
                  <Link key={link.href} href={link.href} onClick={closeMobileMenu} className="rounded px-2 py-2 hover:bg-white hover:text-[#E8621A]">
                    {link.label}
                  </Link>
                ))}
                <Link href="/contato" onClick={closeMobileMenu} className="rounded bg-[#E8621A] px-3 py-2 text-center text-xs font-bold uppercase tracking-[0.05em] text-white">
                  Fale Conosco
                </Link>
              </div>
            </div>
